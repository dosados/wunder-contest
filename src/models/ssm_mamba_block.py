...
from .ssm_model_state import MambaBlockState
...
class MambaBlock(nn.Module):
    ...
    def __init__(
        ...
        self.x_proj = nn.Linear(
            self.d_inner, self.dt_rank + self.d_state * 2, bias=False, **factory
        )
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        Bm, C = B_C.chunk(2, dim=-1)
        y = self.ssm(u=x, delta=dt, B=Bm, C=C, delta_bias=None)
        y = y * self.act(z)
        out = self.out_proj(y)
        return out + hidden_states

    def forward_step(
        self, hidden_states: torch.Tensor, state: MambaBlockState
    ) -> torch.Tensor:
        ...
        x_conv = self.conv1d(seq)[..., -1:].squeeze(-1).squeeze(0)
