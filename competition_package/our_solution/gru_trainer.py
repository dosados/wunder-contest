"""
Трейнер для GRU-модели: обучение GRUModel на ParquetSequenceDataset с валидацией и сохранением весов.
Метрика и лосс — те же, что в trainer.py (weighted Pearson).
"""
import os
import sys
import logging
...
from constants import (
    ...
    VAL_PATH,
    SEQUENCE_BATCH_SIZE,
    WEIGHTS_GRU_DIR as WEIGHTS_DIR,
    EPOCHS_GRU as EPOCHS,
    LR_GRU as LR,
    SAVE_NAME_GRU as SAVE_NAME,
    PRED_CLIP_LOW,
    PRED_CLIP_HIGH,
    WEIGHT_EPS,
    VAR_EPS,
...
def run_training_loop(save_path=None, epochs=None):
    """
    Запускает полный цикл обучения GRUModel.
    Возвращает лучший val weighted_pearson.
