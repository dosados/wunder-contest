"""
Тренер SSM на датасете остатков (plan.txt).

Читает train_residual.parquet и valid_residual.parquet (таргеты = остатки LSTM),
обучает SSM с комбинированным лоссом (как в gym): loss = LAMBDA_L * pearson_loss + (1 - LAMBDA_L) * mse
по предсказаниям и остаткам (таргетам в датасете). Лучшие веса сохраняются в weights_ssm/ssm_model.pt
для использования в check.py и инференсе (pred = LSTM(x) + SSM(x)).
...
# Вес Pearson в комбинированном лоссе: loss = LAMBDA_L * pearson_loss + (1 - LAMBDA_L) * mse (как в gym)
LAMBDA_L = 0.5
...
# Количество прогонов (как в gym для LSTM)
NUM_RUNS = 3


def run_gym_ssm(num_runs=NUM_RUNS, epochs=None):
    if epochs is None:
        epochs = EPOCHS_SSM

    run_folder = f"run_{datetime.now():%d%m%y_%H%M%S}"
    run_dir = os.path.join(WEIGHTS_SSM_DIR, run_folder)
    os.makedirs(run_dir, exist_ok=True)
    ssm_filename = os.path.basename(SAVE_NAME_SSM)  # всегда ssm_model.pt, даже если SAVE_NAME_SSM = "run_xxx/ssm_model.pt"
    best_weights_path = os.path.join(run_dir, ssm_filename)
    # Для check.py по умолчанию: WEIGHTS_SSM_DIR/ssm_model.pt
    default_weights_path = os.path.join(WEIGHTS_SSM_DIR, ssm_filename)
    config_path = os.path.join(run_dir, "config.json")

    run_config = {
        "ssm_constants": _get_ssm_config(),
        "train_data": "train_residual.parquet",
        "valid_data": "valid_residual.parquet",
    }
    os.makedirs(WEIGHTS_SSM_DIR, exist_ok=True)
    ...
    if not os.path.isfile(TRAIN_RESIDUAL_PATH):
        log.error("Не найден датасет остатков: %s. Сначала запустите build_residual_dataset.py", TRAIN_RESIDUAL_PATH)
        return
    ...
    if os.path.isfile(VALID_RESIDUAL_PATH):
        ...
        log.info("Train: %s (%d seq), Valid: %s (%d seq)", TRAIN_RESIDUAL_PATH, len(train_ds), VALID_RESIDUAL_PATH, len(val_ds))
    else:
    ...
    for run in range(1, num_runs + 1):
        ...
        model = SSMModel().to(DEVICE)
        optimizer = torch.optim.Adam(model.parameters(), lr=LR_SSM)
        ...
        for epoch in range(1, epochs + 1):
            ...
            if val_loader is not None:
                ...
                log.info(
                    ...
                    epoch, train_loss, val_loss, val_pearson,
                )
                ...
                if val_pearson > best_val_pearson_global:
                    ...
                    log.info("New best weights → %s (val pearson: %.6f)", default_weights_path, val_pearson)
            else:
                _, train_pearson = validate(
                    model,
                    DataLoader(train_ds, batch_size=SEQUENCE_BATCH_SIZE, shuffle=False, num_workers=0),
                    criterion, DEVICE,
                    total_val_batches=len(train_loader),
                    epoch=epoch, total_epochs=epochs,
                )
                log.info("Epoch %d | train_loss: %.6f | train pearson: %.6f", epoch, train_loss, train_pearson)
                ...
                if train_pearson > best_val_pearson_global:
                    ...
                    torch.save(model.state_dict(), best_weights_path)
                    torch.save(model.state_dict(), default_weights_path)
                    log.info("New best weights → %s (train pearson: %.6f)", default_weights_path, train_pearson)

        log.info("Run %d finished, best val pearson this run: %.6f", run, best_val_pearson_run)

    log.info("All runs finished. Best val pearson (on residual): %.6f. Weights: %s", best_val_pearson_global, default_weights_path)


def _get_ssm_config():
    import constants
    return getattr(constants, "ssm_constants", {}).copy()


def main():
    run_gym_ssm(num_runs=NUM_RUNS, epochs=EPOCHS_SSM)
