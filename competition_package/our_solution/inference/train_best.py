"""
Скрипт: несколько раз с нуля обучает модель с заданным конфигом и сохраняет
лучшие веса (и конфиг) в inference/ для дальнейшего использования PredictionModel.
"""
import os
import sys
import json
...
# our_solution в path до импорта trainer (trainer импортирует model и constants)
...
import constants
from trainer import run_training_loop, WEIGHTS_DIR

# Куда кладём лучшие веса и конфиг для inference
...
RUN_WEIGHTS_NAME = "train_best_run.pt"
RUN_WEIGHTS_PATH = os.path.join(WEIGHTS_DIR, RUN_WEIGHTS_NAME)

# Конфиг по умолчанию: из best_hyperparameters.json или из constants
BEST_HP_PATH = os.path.join(OUR_SOLUTION_DIR, "best_hyperparameters.json")
...
def load_config(config_path: str | None) -> dict:
    ...
    if not os.path.isfile(path):
        log.warning("Config file not found %s, using constants.convo_constants", path)
        return dict(constants.convo_constants)
...
def load_inference_config() -> tuple[dict, float]:
    """
    Загружает inference config.json если есть.
...
def run_multiple_trains(
    ...
    """
    Запускает num_runs обучений с нуля с одним и тем же конфигом.
    Лучшие по val Pearson веса сохраняются в inference/ только при перебитии
    all-time best метрики (хранится в inference/config.json).
    ...
    log.info("Inference config path: %s", INFERENCE_CONFIG_PATH)
    ...
    log.info("Use inference with: config=%s, weights=%s", INFERENCE_CONFIG_PATH, INFERENCE_BEST_WEIGHTS_PATH)
    return all_time_best


def main():
    parser = argparse.ArgumentParser(
        description="Обучить модель несколько раз с одним конфигом и сохранить лучшие веса в inference/"
    )
    parser.add_argument(
        "--runs",
        type=int,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Путь к JSON с конфигом (по умолчанию best_hyperparameters.json в our_solution)",
    )
    parser.add_argument(
        "--epochs",
        type=int,
        default=None,
        help="Эпох за один запуск (по умолчанию из trainer)",
    )
